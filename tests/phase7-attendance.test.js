...
const context = { Date, String, Number, Object, Array, Math, JSON, RegExp, isNaN, Utilities: { formatDate }, getRuntimeSettings_: () => ({ Timezone: "Europe/Vienna" }), readRuntimeRows_: (sheet) => sheet === "Members" ? members : attendanceRows, findMemberById_: (id) => members.find((member) => member.MemberID === id) || null, normalizeMemberId_: (value) => String(value || "").trim().toUpperCase(), isRuntimeActive_: (value) => String(value).toUpperCase() === "ACTIVE", adminString_: (value, maximum) => String(value == null ? "" : value).trim().slice(0, maximum), adminPage_: (value, fallback) => Math.max(1, Number(value) || fallback), adminPageSize_: (value, fallback) => [25, 50, 100].includes(Number(value)) ? Number(value) : fallback, boundedInteger_: (value, fallback, minimum, maximum) => Number.isFinite(Number(value)) ? Math.min(maximum, Math.max(minimum, Math.round(Number(value)))) : fallback, adminInstantText_: (value) => new Date(value).toISOString(), adminError_: (code, message, fields) => { const error = new Error(message); error.adminCode = code; error.adminFields = fields; return error; }, serializeAttendance_: (row) => ({ timestamp: new Date(row.Timestamp).toISOString(), memberId: row.MemberID, firstName: row.FirstName, lastName: row.LastName, trainingType: row.TrainingType, trainingName: row.TrainingName, trainingStart: row.TrainingStart }) };
...
const report = JSON.parse(JSON.stringify(context.getReportData_({ month: "2026-09", threshold: 0 })));
assert.equal(report.totalCheckins, 2); assert.equal(report.uniqueAttendees, 2); assert.equal(report.topAttendees[0].firstName, "Old"); assert.deepEqual(report.lowAttendance.map((row) => row.memberId), ["GYM0003"]);
const client = fs.readFileSync(path.join(apps, "Admin.js.html"), "utf8"), html = fs.readFileSync(path.join(apps, "Admin.html"), "utf8");
for (const expected of ["Attendance", "Reports", "Apply filters", "Clear", "Refresh", "Export page CSV", "Top attendees", "lowAttendance", "member-attendance-next"]) assert.ok(`${html}\n${client}`.includes(expected));
assert.match(client, /\^\[=\+\\-@\]/);
