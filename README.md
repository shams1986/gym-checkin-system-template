...
**Phase 8 — development verification:** the scanner frontend is served from GitHub Pages at `https://shams1986.github.io/gym-checkin-system-template/` and calls the Phase 8 Apps Script backend through the existing JSONP contract. Camera access passed a manual desktop test from the GitHub Pages URL. The Apps Script scanner UI was removed because camera access must not run inside Apps Script HtmlService; Apps Script remains the config/check-in backend, while admin remains a separate restricted deployment. Development Sheet setup/schema verification passed. Admin authentication uses an Apps Script deployment that executes as the accessing user. The current development admin deployment is restricted to **Only myself** (`salahadin35@gmail.com`): the allowed owner can open the dashboard, while non-owner and incognito access is blocked before dashboard access or Google permission consent.

Remaining optional Phase 8 manual QA is a physical phone/tablet scan, a real QR-card success/duplicate flow, and PWA installation plus offline refresh. A future multi-admin/client access architecture remains a separate product-design task. The next stage is **Phase 9 — first client instance from the template**.
