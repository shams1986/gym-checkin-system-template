...
- `WebApp.gs`: public JSON/JSONP routing; the root is a backend descriptor and never a camera UI
...
Phase 5 authentication and UI setup are documented in `docs/ADMIN_SETUP.md`. The admin uses a separate web-app deployment that executes as the accessing user; no operational data or mutation is available until `adminApi` verifies `Session.getActiveUser().getEmail()` against the protected allowlist. The anonymous backend deployment stays separate and serves only the public config/check-in contract. The camera frontend is hosted on static HTTPS, never in HtmlService. Implementation handlers end in `_` and cannot be called directly through `google.script.run`.
